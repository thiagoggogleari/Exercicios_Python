...
#     a.Cada funcionário receberá o equivalente a 20% do seu salário bruto de dezembro;
      ...
      #  Um valor de salário igual a 0 (zero) encerra a digitação. Após a entrada de todos os dados o programa deverá calcular o valor do abono
...
# Quantidade de pessoas que receberam um abono de R$100
contadorSalarioMinimo = 0

print('\nSalário    - Abono')

for i in range(len(salarios)):
  abono = round(salarios[i] * 0.20,2)
  if abono < 100:
    abono = 100.00
    contadorSalarioMinimo += 1

  todosAbonos.append(abono)    
  
  # Tamanho de espaçamento da tabela
  espaco1 = 7 - len(str(salarios[i])) 
  espaco2 = 6 - len(str(abono))
  
  print('R${}{:.2f} - R${}{:.2f}'.format(' '*espaco1,salarios[i],' '*espaco2,abono))
  
print('\nForam processados {} colaboradores'.format(len(salarios)))

# Total de R$ pago em abono
somatoriaAbonos = 0

for j in range(len(todosAbonos)):
  somatoriaAbonos = somatoriaAbonos + todosAbonos[j]
  
print('Total gasto com abonos: R${:.2f}'.format(somatoriaAbonos))
print('Valor mínimo pago à {} colaboradores'.format(contadorSalarioMinimo))

# Verifica o maior salário
listaOrdenada = sorted(salarios)
maiorSalario = listaOrdenada[len(x)-1]
print('Maior valor de abono pago: R${:.2f}'.format(maiorSalario))
